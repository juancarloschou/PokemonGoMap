...
begin
	...
	into #mgpr --max generador pokemon ratio
	...
	select r.idGenerador, r.idPokemon,
			...
			convert(varchar,ra.idTipo) as propiedad, --idTipoPokemon
			@idZona 
	from
	(
	...
	select r.idGenerador, r.idPokemon,
			convert(varchar, r.idPokemon) + ',' + convert(varchar, veces) + ',' + convert(varchar, intentos) + ',' + convert(varchar, convert(decimal(5,2),r.ratio*100.0)) as propiedad, --idPokemon2, veces, intentos, probabilidad
			@idZona
	from
	(	
	...
	select r.idGenerador, r.idPokemon,
			convert(varchar, r.idPokemon) + ',' + convert(varchar, veces) + ',' + convert(varchar, intentos) + ',' + convert(varchar, convert(decimal(5,2),r.ratio*100.0)) as propiedad, --idPokemon3, veces, intentos, probabilidad
			@idZona
	from
	(
		select idGenerador, substring(substring(propiedad, CHARINDEX(',',propiedad)+1, len(propiedad)), CHARINDEX(',', 
	...
	into #ghif --generador hora ini fin
	from Generador g
	inner join GeneradorPokemon p
	on g.idGenerador = p.idGenerador
	...
	select p.idGenerador, p.idPokemon, --idPokemon1
			isnull(HoraIniFin,'') + ',' + 
			...
			+ ',,' as propiedad, --horaGenerador, ultimaExploracion, Fiabilidad
			@idZona 
	...
	--ultima vez visto el pokemon primario, podria poner ultima vez visto un pokemon en el generador para saber si esta actualizado, pero asi se sabe a que hora sale el primario
	where idZona = @idZona
	--datos del generador, propiedades de hora, ultima y fiabilidad



	--mete en el principal las propiedades del primario, secundario y terciario, y los idPokemon2 y 3
	...
	set propiedad = isnull(pp.propiedad,',,,') + 
		...
		idPokemon2 = s.idPokemon,
		idPokemon3 = t.idPokemon
	...
	--poner propiedad
	insert into FT_Parada_min(id, lon, lat, idZona, titulo, fecha)
